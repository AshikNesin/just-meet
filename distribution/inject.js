(function(modules) {
  ...
  const createDelay = ({clearTimeout: defaultClear, setTimeout: set, willResolve: willResolve}) => (ms, {value: value, signal: signal} = {}) => {
  ...
  delay.createWithTimers = ({clearTimeout: clearTimeout, setTimeout: setTimeout}) => {
    const delay = createDelay({
  ...
  var options_storage = __webpack_require__(0);
  const autoMuteOnJoin = async () => {
    const options = await options_storage["a"].getAll();
    const [micBtnEl] = [ ...document.querySelectorAll('[role="button"]') ];
    if (options.autoMuteOnJoin) {
      micBtnEl.click();
    }
  };
  var auto_mute_on_join = autoMuteOnJoin;
  ...
  var auto_turn_off_cam_on_join = autoTurnOffCamOnJoin;
  var delay = __webpack_require__(3);
  var delay_default = __webpack_require__.n(delay);
  ...
  var auto_join_call = autoJoinCall;
  const readyStateCheckInterval = setInterval((function() {
    if (document.readyState === "complete") {
      clearInterval(readyStateCheckInterval);
      auto_mute_on_join();
      auto_turn_off_cam_on_join();
      auto_join_call();
    }
  }), 10);
} ]);